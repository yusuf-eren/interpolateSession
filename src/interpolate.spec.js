...
describe('interpolate extra tests by Yusuf Eren', () => {
  it('should interpolate with different delimiters', () => {
    ...
    const expected = 'Hi Yusuf, how are you today?';
    expect(result).toEqual(expected);
  });

  it('should return input if values with different delimiters are not matching', () => {
    const result = interpolate(
      'Hi {£$firstname$#, how are you today?',
      { firstname: 'Yusuf' },
      {
  ...
  it('should work with complex string delimiters', () => {
    const result = interpolate(
      'Hi $#$#$#$€@₺firstname5675623axarweq, what`s going on?',
      { firstname: 'Yusuf' },
      { leftDelimiter: "$#$#$#$€@₺", rightDelimiter: "5675623axarweq"}
    );
    const expected = 'Hi Yusuf, what`s going on?'
    expect(result).toEqual(expected)
  });

  it('', () => {
    const expected = 'You are using delimiters on wrong sides';
    expect(() => {
      interpolate(
        'Hi >$£firstname$$#, are you using delimiters on correct sides??',
        { firstname: 'Yusuf' },
